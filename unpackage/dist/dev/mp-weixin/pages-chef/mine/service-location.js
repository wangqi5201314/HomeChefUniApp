...
const _sfc_main = {
  ...
  methods: {
    ...
    async loadServiceLocation() {
      this.loading = true;
      try {
        const data = await api_chefServiceLocation.getChefServiceLocation();
        if (!data || !data.id) {
          this.initDefaultRegion();
          return;
        }
    ...
    async handleSubmit() {
      ...
      this.saving = true;
      try {
        await api_chefServiceLocation.saveChefServiceLocation(this.buildSubmitPayload());
        common_vendor.index.showToast({
          title: "保存成功",
          icon: "success"
        });
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    g: $data.searchResults.length
  }, $data.searchResults.length ? {
    h: common_vendor.f($data.searchResults, (item, index, i0) => {
      return {
    ...
    j: $data.mapLongitude,
    k: $options.markers,
    l: common_vendor.o((...args) => $options.handleMapTap && $options.handleMapTap(...args)),
    m: common_vendor.t($options.selectedLocationText),
    n: common_vendor.t($data.form.province || "请选择省份"),
    o: !$data.form.province ? 1 : "",
    p: $options.provinceRange,
    q: $options.provinceIndex,
    r: common_vendor.o((...args) => $options.handleProvinceChange && $options.handleProvinceChange(...args)),
    s: common_vendor.t($data.form.city || "请选择城市"),
    t: !$data.form.city ? 1 : "",
    v: $options.cityRange,
    w: $options.cityIndex,
    x: common_vendor.o((...args) => $options.handleCityChange && $options.handleCityChange(...args)),
    y: common_vendor.t($data.form.district || "请选择区县"),
    z: !$data.form.district ? 1 : "",
    A: $options.districtRange,
    B: $options.districtIndex,
    C: common_vendor.o((...args) => $options.handleDistrictChange && $options.handleDistrictChange(...args)),
    D: common_vendor.t($data.form.town || ($options.townOptions.length ? "请选择镇/街道" : "暂无可选镇街")),
    E: !$data.form.town ? 1 : "",
    F: $options.townRange,
    G: $options.townIndex,
    H: common_vendor.o((...args) => $options.handleTownChange && $options.handleTownChange(...args)),
    I: $data.form.detailAddress,
    J: common_vendor.o(($event) => $data.form.detailAddress = $event.detail.value),
    K: common_vendor.t($data.saving ? "保存中..." : "保存服务位置"),
    L: $data.saving,
    M: $data.saving || $data.loading,
    N: common_vendor.o((...args) => $options.handleSubmit && $options.handleSubmit(...args))
  }));
}
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-c87078e4"]]);
wx.createPage(MiniProgramPage);
//# sourceMappingURL=../../../.sourcemap/mp-weixin/pages-chef/mine/service-location.js.map
