...
export function chatWithAiStream(options = {}) {
  const payload = {
  ...
  function emitFrame() {
    ...
    if (eventName === 'message') {
      ...
      options.onError &&
        options.onError(new Error(payloadData && payloadData.message ? payloadData.message : 'AI请求失败'))
      return
    }
  ...
  const requestTask = wx.request({
    ...
    success: (response) => {
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        options.onError &&
          options.onError(new Error(`AI请求失败(${response.statusCode})`))
        return
      }
    ...
    fail: (error) => {
      ...
      options.onError &&
        options.onError(new Error((error && error.errMsg) || '网络异常'))
    }
  })
