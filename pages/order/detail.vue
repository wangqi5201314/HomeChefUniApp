<template>
	<view class="page">
		...
		<view v-if="showActionBar" class="bottom-bar">
			<button v-if="showCancelButton" class="action-btn secondary" :loading="cancelSubmitting"
				:disabled="cancelSubmitting || paying" @click="openCancelPopup">
				取消订单
			</button>
			<button v-if="showPayButton" class="action-btn primary" :loading="paying"
				:disabled="paying || cancelSubmitting" @click="handlePay">
				立即支付
			</button>
			<button v-if="showReviewButton" class="action-btn primary" @click="goReview">
				去评价
			</button>
		</view>
		...
		<view v-if="showCancelModal" class="modal-mask" @click="closeCancelPopup">
			<view class="modal-card" @click.stop>
				...
				<view class="modal-actions">
					...
					<button class="modal-btn danger" :loading="cancelSubmitting" :disabled="cancelSubmitting"
						@click="submitCancel">
						确认取消
					</button>
				</view>
		...
		computed: {
			...
			showPayButton() {
				return this.orderDetail.orderStatus === 'WAIT_PAY' ||
					this.orderDetail.orderStatus === 'PENDING_CONFIRM'
			},
			showReviewButton() {
